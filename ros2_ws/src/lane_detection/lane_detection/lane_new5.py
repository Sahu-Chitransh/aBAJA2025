...
class LaneDetectorNode(Node):
    ...
    def preprocess_image(self, frame):
        ...
        blur = cv2.GaussianBlur(combined_mask, (7, 7), 0)
    ...
    def fit_polynomial(self, binary_warped):
        ...
        peaks = np.argpartition(histogram, -3)[-3:]  # top 3 peaks, likely left, center, right lanes
        peaks = sorted(peaks)
        mid = binary_warped.shape[1] // 2
        leftx_base = min(peaks, key=lambda x: abs(x - mid//2))
        rightx_base = min(peaks, key=lambda x: abs(x - 3*mid//2))
        ...
        left_lane_inds = track_lane(leftx_base)
        right_lane_inds = track_lane(rightx_base)
